...
Pattern 2 also extends cleanly to deliverables typed as `DmaicRow[]` (`msaValidation`, `gutPrioritization`, `vitalXs`, `actionPlan`, `controlPlan`) even though `DmaicRow = {[key: string]: string}` gives no compile-time field guarantee. Define a local concrete row type + `EMPTY_*_ROW` default matching the actual Gemini prompt schema (check `DMAIC_SYSTEM_PROMPT` in the api-server route, not just the generated types), then a generic `normalizeRow`/`normalizeRows` helper to coerce loose `DmaicRow[]` into the typed shape before rendering. TS structural typing lets the concrete row type flow back into `DmaicRow[]` on save without casting, since every field is a string.
