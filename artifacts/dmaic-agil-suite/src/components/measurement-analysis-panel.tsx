  ...
  return <section data-testid="panel-measurement-analysis" className="reveal mt-8 space-y-5">
    <div className="overflow-hidden rounded-2xl border border-chart-3/25 bg-card shadow-[0_18px_55px_-38px_hsl(var(--chart-3)/0.55)]">
      ...
      {dataset && analysis && <div className="space-y-7 p-5 sm:p-6">
        ...
        <div className="space-y-5">{analysis.variables.map((variable) => <article key={variable.name} className="rounded-2xl border border-border bg-background/70 p-4 sm:p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-chart-3">Unidade do agrupamento</p><h3 className="mt-1 text-lg font-black">{variable.name}</h3></div><span className={`rounded-full px-3 py-1 text-[10px] font-bold ${variable.normality === 'Não normal' ? 'bg-destructive/10 text-destructive' : 'bg-primary/10 text-primary'}`}>{variable.normality}</span></div><div className="grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-border bg-card p-3"><p className="mb-2 text-xs font-bold">Valor por {analysis.xColumn}</p><SequenceChart variable={variable} labels={analysis.xValues} /></div><div className="rounded-xl border border-border bg-card p-3"><p className="mb-2 text-xs font-bold">Histograma de Valor por {analysis.groupingColumn}</p><Histogram variable={variable} /></div></div><div className="mt-4 grid gap-3 lg:grid-cols-2"><p className="rounded-xl bg-muted/60 p-3 text-xs leading-relaxed"><strong>Variabilidade de Valor.</strong> {variable.variabilityComment}</p><p className="rounded-xl bg-muted/60 p-3 text-xs leading-relaxed"><strong>Padrão sequencial.</strong> {variable.sequenceComment}</p></div></article>)}</div>
